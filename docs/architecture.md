...
default   轻量通用任务
parse     文档解析、分块、向量化
memory    记忆提取、去重、聚类和巩固
research  深度研究长任务
notify    消息通知
```

Beat 只负责产生周期任务，不执行重任务。每个任务携带 `job_id`、`request_id`、用户归属和幂等键。
...
- 输入输出 token、模型、供应商和估算费用
- 检索候选、融合得分和最终引用
- Celery 排队时间、执行时间和失败阶段

评测从 RAG 核心闭环开始，包括：解析正确性、Recall@K、MRR/nDCG、引用命中率、回答忠实度、首 token 延迟和单次成本。评测集与生产数据分离并版本化。
...
当前只建设 Phase 0，不提前创建后续业务空壳。Phase 0 完成后立即实现 Phase 1，再以“可运行、可测试、可演示”为标准逐阶段推进。
