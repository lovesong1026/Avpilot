...
项目已完成 v0.3 可追溯流式问答：知识库检索已接入百炼模型，回答通过 SSE 实时生成，并同步返回可展开的原文引用；会话、消息和引用快照都会持久化。

下一条交付主线是 v0.4 多模态与标签：补齐网页、图片入库和 AI 自动标签，形成文本与视觉内容统一检索闭环。
...
| v0.4 多模态与标签 | 📋 计划中 | 支持网页与图片入库，自动生成图片描述、OCR、物体和场景；自动分类并优先复用已有标签。 |
