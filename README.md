...
- [v1.0 深度研究闭环](docs/v1.0-deep-research.md)

## 当前阶段

项目已完成 v1.0 深度研究闭环：用户可以独立创建研究任务，选择知识库、长期记忆和
联网搜索作为证据源。后台会自动制定研究计划、逐个执行子问题、沉淀证据和研究笔记，
通过 Verifier 发现缺口并有限补充检索，最终生成带编号引用的 Markdown 报告。任务由
Celery `research` 队列可靠执行，支持进度轮询、失败重试、Worker 中断恢复和报告导出。
研究过程只记录 Token 用量，不估算或保存模型费用。
...
| v1.0 深度研究 | ✅ 已完成 | 建立独立研究页面和可靠任务状态机；自动规划子问题，联合知识库、记忆与联网搜索收集证据，经 Verifier 补充检索后生成并导出带引用的 Markdown 报告。 |
...
- 新增独立“深度研究”一级页面；智能问答中的当前问题可以携带知识库和联网设置转入
  深度研究。
- PostgreSQL 保存研究任务、研究步骤与引用证据，所有读取和写入均按用户隔离。
- 研究规划器把复杂课题拆成 3～6 个可检索子问题，逐项搜索知识库、长期记忆和可选
  互联网，并形成受证据约束的研究笔记。
- Verifier 检查关键问题覆盖度、证据缺口和来源冲突；证据不足时最多进行三轮有限
  补充检索，避免无限循环。
- 最终报告使用 `[n]` 引用证据库，明确披露风险和不确定性，并可导出 Markdown。
- 报告完成后确定性校验引用编号；证据为空时生成“证据不足”报告，不产生占位引用。
- 独立 `research` 队列通过 Transactional Outbox 投递；任务支持自动重试、手动重试、
  Worker 中断恢复和页面轮询进度。
- 研究上下文最多保留 40 条去重证据，并限制进入模型的引文长度，控制上下文规模。
- 只累计输入、输出和总 Token，不维护价格表或费用字段。
...
uv run celery -A app.worker.celery_app worker -l INFO \
  -Q ingestion,memory,research,maintenance --concurrency=2
